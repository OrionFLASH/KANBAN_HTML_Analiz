...
def test_load_unified_team_files(tmp_path: Path) -> None:
    ...
    frame: pd.DataFrame = pd.DataFrame(
        {
            ...
            "ID ПрПр": ["L1", "L1", "L2", "L1"],
            "ID сделки": ["D1", "D1", "-", "D1"],
            "Участник команды": ["Лидер Лида", "Лидер Сделки", "-", "Участник НеЛидер"],
            "Роль участника команды": ["ПС", "ВКС", "-", "КС"],
            "Лидер": ["Да", "Да", "-", "Нет"],
            "ТБ": ["ТБ1", "ТБ1", "-", "ТБ1"],
        }
    )
    ...
    assert "Участник НеЛидер" not in set(lead_df["Участник команды"].astype(str))
    assert len(deal_df) == 1
