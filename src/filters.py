...
def is_exclude_filter(flt: dict[str, Any]) -> bool:
    """True — фильтр исключает строки по подстроке в колонке (терминальные стадии сделки)."""
    return flt.get("filter_mode") == "exclude" or "exclude_contains" in flt
...
def exclude_match_mask(
    ...
    """
    Маска строк, которые нужно исключить (подстрока в «Стадия сделки»).
    Пустая стадия сделки (—, nan) никогда не считается терминальной.
    """
    token: str = str(flt.get("exclude_contains", "")).strip()
    if not token:
        return pd.Series(False, index=series.index)
    case: bool = bool(flt.get("case_sensitive", False))
    text: pd.Series = series.fillna("").astype(str).str.strip()
    match: pd.Series = text.str.contains(token, case=case, na=False)
...
def filter_terminal_deal_stage_rows(
    ...
    """
    Убирает только строки с терминальной «Стадией сделки» (конкретная дата отчёта).
    ...
    for name in names:
        ...
        if not column or column not in df.columns:
            continue
        keep &= row_keep_mask(df, column, flt, config)

    before: int = len(df)
    result: pd.DataFrame = df.loc[keep].reset_index(drop=True)
    if len(result) < before:
        logger.info(
            "Терминальные стадии сделки: %d → %d строк (%s)",
            before,
            len(result),
            ", ".join(names),
        )
...
def _apply_filter_subset(
    ...
    *,
    ...
    for name, flt in filters_cfg.items():
        ...
        mask: pd.Series = row_keep_mask(result, column, flt, config)
