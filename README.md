...
Готовый пакет Excel v2 (код, `config_excel_v2.json`, Docs): **`v2_program_files_20260911b.zip`** в корне репозитория. В архиве: `run_excel.py` / `run.py`, **весь `src/`** (общие модули + `v1` + `v2`, **без** `src/Tests/`), конфиги, Docs; старые `v2_*.zip` удалены. Справочник конфига: [Docs/CONFIG_EXCEL_V2.md](Docs/CONFIG_EXCEL_V2.md) v2.9.0; полный разбор **каждого** ключа: [Docs/CONFIG_EXCEL_V2_PARAMS.md](Docs/CONFIG_EXCEL_V2_PARAMS.md).

Отдельный архив документации (папка `Docs/` + `README.md` + `ROADMAP.md`): **`docs_20260911b.zip`**.
