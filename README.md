...
- **Агрегация в HTML:** переключатель «По продуктам» / «По группам» — данные берутся из соответствующего среза JSON (`visualizations.aggregations`)
- `config.product_analysis_mode` — только для **Excel**; в JSON всегда **обе** агрегации
...
| 0.8.0 | 2026-08-31 | JSON: обе агрегации (продукт+группа); HTML — выбор среза; ТБ вертикально; Excel по config |
