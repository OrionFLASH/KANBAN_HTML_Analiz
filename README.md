...
- Вкладки **Графики** и **Сводная матрица**
- **Графики:** режим «свод + каждый» — сверху все выбранные ТБ или **группы** (при `group_only`) / продукты на одном графике, ниже отдельная карточка на каждый элемент
- Режим `group_only` в JSON: агрегация по **группам**, фильтр продуктов скрыт, строки матрицы — группы
- Режим «По ТБ»: графики **друг под другом** (одна колонка) — больше места для легенды
- В JSON (`meta.filters_applied`) — какие pipeline-фильтры уже применены к данным
...
| 0.8.0 | 2026-08-31 | `group_only`: агрегация JSON по группам (row_key), HTML — графики по группам, ТБ вертикально |
