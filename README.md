...
Готовый пакет Excel v2 (код, `config_excel_v2.json`, Docs): **`v2_program_files_20260908d.zip`** в корне репозитория (без тестов; старые `v2_*.zip` удалены).
...
| Лист | Содержание |
|------|------------|
| **Нормативы** | Min/max, P20/P50/P80 по ТБ и «все тб»; по строке — до/после и отсечено по каждому вх. фильтру + выбросы |
| **Статистика** | Воронка фильтров (до/после) + свод отсечений по группам |
| **Распределение сроков** | Матрица группа/продукт × дни (число лидов); градиент; пустые вместо 0 |
| **Уникальные ID** | Снимок лидов, лидеры (+ почты Альфа/Сигма по ТН), норматив P{n}, превышение |
...
| 2.3.5 | 2026-09-08 | Fix лидеров без «Дата отчета» в файле команды; архив `20260908d` |
