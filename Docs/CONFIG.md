...
| `lead_team.test` / `.prod` | Массив файлов «Команда лида …» — все склеиваются; лидер лида (`Лидер = Да` по max дате на `ID ПрПр`) |
| `deal_team.test` / `.prod` | Массив файлов «Команда сделки …» — все склеиваются; лидеры сделки по `ID сделки` |
