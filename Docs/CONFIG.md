...
| `prod_files` | array | prod Kanban-файлы (ЕФС) |
...
| `input_prod` | `IN/PROD` | Prod Excel (Kanban ЕФС + команда) |
...
Имена Kanban-файлов prod (ЕФС0/ЕФС1 по продуктам и стадиям). Полный список — в `config.json` / `config_excel_v2.json` → `prod_files`.
...
|-------|---------|-----------------|
| `test` | `paths.input_test` | `test_files` + `team_files.files.test` или legacy `lead_team`/`deal_team` |
| `prod` | `paths.input_prod` | `prod_files` + `team_files.files.prod` (или legacy) |
...
**Prod (текущий config):** 11 Kanban ЕФС + 4 объединённых файла «команда лида и сделки» = **15 файлов** в `IN/PROD/`.
...
| Графики распределения | Гистограмма (лиды × срок), ECDF (% лидов), ранговая шкала; линии П20/50/80 (как в backend) |
...
Файлы: `index.html`, `css/dashboard.css`, `js/data.js`, `js/distribution.js`, `js/icons.js`, `js/multi-filter.js`, `js/managers.js`, `js/charts.js`, `js/chart-expand.js`, `js/pivot.js`, `js/app.js`.
...
### Режимы графика (HTML, селект «Режим графика»)

| Значение | Тип | Источник данных |
|----------|-----|-----------------|
| `by_product` | распределение | Основной JSON — `distribution_series`; карточка: гистограмма + ECDF + ранговая шкала |
| `by_tb` | распределение | То же; свод и детализация по ТБ |
...
**Перцентили на графиках:** считаются в браузере из `days_sorted` тем же эмпирическим методом, что `src/percentile_stats.py` (нижние p% лидов на отсортированной шкале). **П80 = X** означает: срок границы, ниже которой по счёту укладываются нижние 80% лидов (≤ X), не «80% лидов дольше X».

**Разворот:** клик по области графика (`.dist-panel` или bar-wrap) — увеличение одного графика в область `.main` (между боковыми панелями). Клик по заголовку карточки, чипам перцентилей или промежутку между графиками — вся карточка на весь экран с затемнением. Модули: `distribution.js`, `chart-expand.js`.
...
Excel с командой (лежат рядом с kanban в `paths.input_*`).

**Рекомендуемый формат (единый комплект):** файлы содержат и команду лида, и команду сделки; различие — колонка **«Тип команды»**:

| Значение | Смысл |
|----------|--------|
| `1` | строки команды лида |
| `2` | строки команды сделки |
| `-` (и ТН тоже `-`) | лид не взят в работу — лидеры не подливаются; КМ/ВКС берутся из канбана |

| Ключ | Описание |
|------|----------|
| `enabled` | Включить подгрузку |
| `files.test` / `.prod` | Единый список файлов «команда лида и сделки» — склеиваются, затем split по «Тип команды» |
| `lead_team` / `deal_team` | Legacy: раздельные списки (используются, если `files` для mode пуст) |
| `team_type_values` | Допустимые значения типа: `lead` / `deal` / `unassigned` |
| `leader_values` | Значения «да» для колонки «Лидер» |
| `columns` | Имена колонок файла команды (в т.ч. `team_type`) |

**Отбор лидера:** `Лидер = Да`, max `Дата отчета`, затем max `Дата добавления в команду` на `ID ПрПр` (лид) или `ID сделки` (сделка).

**Команда зависшего лида (уникальные ФИО):**

1. Лидер(ы) лида (тип=1)
2. Лидер(ы) сделки (тип=2)
...
"team_files": {
  ...
  "files": {
    "test": [],
    "prod": [
      "Канбан ЕФС0 команда лида и сделки __07-09-2026.xlsx",
      "Канбан ЕФС1 команда лида и сделки (ФОТ) __07-09-2026.xlsx"
    ]
  },
  "lead_team": {
