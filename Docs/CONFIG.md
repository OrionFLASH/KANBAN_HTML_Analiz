...
| `lead_team.test` / `.prod` | Массив файлов «Команда лида …» — все склеиваются; лидер лида (`Лидер = Да`, max `Дата отчета`, затем max `Дата добавления в команду` на `ID ПрПр`) |
| `deal_team.test` / `.prod` | Массив файлов «Команда сделки …» — все склеиваются; лидеры сделки по `ID сделки` (тот же отбор дат) |
