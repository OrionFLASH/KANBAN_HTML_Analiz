...
**Версия документа:** 2.9.3 (2026-09-11)
...
| `split_by_tb_over_rows` | Если строк **больше** порога — разбиение по значениям колонки ТБ |
| `split_mode` | `files` (по умолчанию) — **отдельный xlsx на каждый ТБ** (меньше пик I/O/RAM); `sheets` — несколько листов в одном файле (старое поведение, тяжело на 2M+ строк) |
| `sheet_name_prefix` | Префикс в имени листа (`sheets`) или суффиксе файла (`files`): `…_percentiles_ТБ_ЦЧБ_….xlsx` |

При числе строк ≤ порога — один файл с листом `output.sheets.percentiles` («Строки для процентилей»).

Запись в режиме `files` идёт **последовательно** по ТБ с освобождением памяти между файлами.
