...
**Версия документа:** 2.2.7 (2026-09-03)
...
На листах «Свод по менеджеру» / «Свод ПрПр…» добавляются колонки `Почта Альфа` / `Почта Сигма` сразу после «Табельный номер».
...
| Оформление | Шапка и названия продуктов — жирный + бледно-жёлтый; продукт влево; стр.3 — номера колонок + автофильтр; `freeze_panes: D4` |
...
| `freeze_panes` | `D4` | Закрепить A–C, шапку, итоги и строку автофильтра |
