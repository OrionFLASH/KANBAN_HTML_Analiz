...
| `duration_matrix` | Распределение сроков | Матрица группа/продукт × дни; колонки P20/P50/P80 (пересчёт по всем стадиям); «Всего»; выделение дня порога (`exceedance.percentile`); freeze до «Всего» |
...
"team_files": {
  ...
  "files": {
    ...
    "prod": [
      ...
      "Канбан ЕФС1 команда лида и сделки (Активы-Пассивы) __07-09-2026.xlsx",
      "Канбан ЕФС1 команда лида и сделки (SberX-НКД) __07-09-2026.xlsx"
    ]
  },
...
- После склейки `files` строки делятся по «Тип команды»; лидер — `Лидер` ∈ `leader_values` на max(`Дата отчета`), затем max(`Дата добавления в команду`); если `Дата отчета` нет в файле — только max(`Дата добавления в команду`); равные даты — все через `\n`.
...
| Ось | Содержание |
|-----|------------|
| Строки | «Группа продукта», «Продукт» — алфавит или по объёму; продукты внутри групп |
