...
| Ключ | Назначение |
|------|------------|
| `norms` | Таблица нормативов + колонки отсечения выбросов **по строке группы**; `freeze_panes` / автофильтр как у остальных табличных листов |
| `statistics` | Воронка фильтров и свод выбросов (отдельное оформление: два блока) |
