...
**Версия документа:** 2.4.2 (2026-09-10)
...
| `excel_theme` | `"green_red"` — заливка колонок с маркерами «Мин»/«Макс» (см. `excel_format` / `light_format_sheets`) |
...
На листе **«Уникальные ID»** (файл detail) после «Стадия работы с лидом» добавляются колонки — **имя = вариант «Текущий статус»**, значение = срок лида на этой стадии (дни). Нет срока — пустая ячейка.
...
| Ключ | Описание |
|------|----------|
| `order` | Канонический порядок колонок (всегда выводятся) |
| `include_others` | После `order` — остальные статусы из данных |
| `others_sort` | `alpha` — А→Я |

Срок берётся из `lead_stage_records` (уровень status): при нескольких продуктах/ТБ у одного лида — **max** дней. Сопоставление имён статусов — без учёта регистра.
