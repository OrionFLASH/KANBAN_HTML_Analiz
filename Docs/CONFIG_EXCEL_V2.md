...
- `green_red` — зелёный min, красный max (на листах из `light_format_sheets` отключено)
- `light_format_sheets` — ключи больших листов (`leads`, `violations`): только freeze/автофильтр/шапка/ширина, без поклеточного оформления (ускоряет экспорт ПРОД)
