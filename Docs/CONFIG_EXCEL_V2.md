...
**Версия документа:** 2.2.9 (2026-09-03)
...
| `duration_matrix` | Распределение сроков | Матрица группа/продукт × дни; `sort_mode` (сейчас `by_volume`); «Всего» + строка номеров/автофильтр; freeze `D4` |
...
- Лидер — строка с `Лидер` ∈ `leader_values` на max(`Дата отчета`) по `ID ПрПр` (lead) или `ID сделки` (deal); при нескольких на одной дате отчёта — max(`Дата добавления в команду`); при равных датах добавления — все через `\n`.
...
- Несколько лидеров (равные даты) — значения в одной ячейке через **перевод строки** (`\n`).
...
На листах «Свод по менеджеру» почты вставляются сразу после «ФИО»; на «Свод ПрПр…» — после «Табельный номер».
...
| Ось | Содержание |
|-----|------------|
| Строки | «Группа продукта», «Продукт» — алфавит; продукты внутри групп; колонка «Всего» |
| Строка 2 | Горизонталь «Всего» — сумма лидов по каждому дню + общий итог |
| Столбцы | Целые дни, где есть **хотя бы один** лид (пустые дни между min и max не выводятся) |
| Ячейка | Число лидов с этим сроком у продукта; **0 и пусто не пишутся** |
| Оформление | Шапка и названия продуктов — жирный + бледно-жёлтый; продукт влево; стр.3 — номера колонок + автофильтр; закрепление — `sheet_freeze.duration_matrix` |
...
"duration_matrix": {
  ...
  "max_day_span": 3000,
  "color_scale": { "start": "63BE7B", "mid": "FFEB84", "end": "F8696B" }
}
```
...
| `sort_mode` | `by_volume` | `alpha_days` — группы/продукты А→Я, дни по возрастанию; `by_volume` — сверху/слева максимум лидов |
