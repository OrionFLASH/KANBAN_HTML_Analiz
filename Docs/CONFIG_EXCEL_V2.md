...
- После склейки `files` строки делятся по «Тип команды»; лидер — `Лидер` ∈ `leader_values` на max(`Дата отчета`), затем max(`Дата добавления в команду`); равные даты — все через `\n`.
