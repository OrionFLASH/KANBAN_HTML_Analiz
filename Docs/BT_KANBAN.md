...
| Отказ (стадия сделки) | `Стадия сделки` | исключить строки с «отказ» (`exclude_deal_otkaz`) |
...
> **Решение (v1.0.6):** терминальные стадии сделки — **построчное** исключение на этапе трекинга (лид × дата отчёта), не отсекают весь `ID ПрПр`. Пустая «Стадия сделки» не фильтруется. По умолчанию все три исключения **ВКЛ** в UI и Excel.
...
## 9. Согласованные решения (v1.0.6, 2026-08-31)
...
| Терминальные стадии | `exclude_deal_*` — построчно по «Стадия сделки»; HTML + JSON комбинации; default_active: все три ВКЛ |
| Excel P80 КМ | Колонки «П80 КМ ≥» / «П80 КМ ≥ сд.» — уникальные КМ с сроком ≥ P80 в группе |
...
- [v] Фильтры: изменение условий, ввод данных, ЕФС (config-only), метка (2 варианта), терминальные стадии сделки (построчно)
- [v] Excel: колонки «П80 КМ ≥» для метрик days_on_stage / days_since_deal
